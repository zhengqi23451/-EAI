...
@pytest.fixture(scope="module")
def navigate_to_category_management(driver, login):
    """导航到类别管理页面"""
    ...
    try:
        ...
        #点击类别管理
        wait.until(EC.element_to_be_clickable((By.XPATH,'//div[@class="el-scrollbar__view"]//span[text()="类别管理"]'))).click()

        # 验证页面加载
        time.sleep(2)
        if driver.current_url!="http://192.168.2.42:9529/#/sys/manage/webconfig/category" :
            allure.attach(driver.get_screenshot_as_png(), name="导航类别管理页面失败截图",attachment_type=allure.attachment_type.PNG)
        assert driver.current_url == "http://192.168.2.42:9529/#/sys/manage/webconfig/category"
    except Exception as e:
        raise e
...
@allure.feature("网站配置管理")
@allure.story("类别管理")
@allure.description("新增类别")
def test_add(driver,navigate_to_category_management):
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        #点击新增按钮
        ...
        wait.until(EC.element_to_be_clickable(
            (By.XPATH, '//div[@class="el-dialog__body"]/form/div[label[@for="web_name"]]//input'))).send_keys("test")
        #点击保存按钮
        wait.until(EC.element_to_be_clickable(
        ...
        if text.text != "新增成功":
            ...
            allure.attach(driver.get_screenshot_as_png(), name="新增类别失败截图",
                          attachment_type=allure.attachment_type.PNG)
...
def test_add2(driver,navigate_to_category_management):
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        #点击新增按钮
        ...
        #输入配置标识
        wait.until(EC.element_to_be_clickable(
            (By.XPATH, '//div[@class="el-dialog__body"]/form/div[label[@for="conf_key"]]//input'))).send_keys("test1")
        #输入配置值
        wait.until(EC.element_to_be_clickable(
            (By.XPATH, '//div[@class="el-dialog__body"]/form/div[label[@for="conf_value"]]//input'))).send_keys("test1")
        #输入配置标题
        wait.until(EC.element_to_be_clickable(
            (By.XPATH, '//div[@class="el-dialog__body"]/form/div[label[@for="conf_title"]]//input'))).send_keys("test1")
...
def test_modify2(driver,navigate_to_category_management):
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        # 选择最后一个类别
...
def test_delete2(driver,navigate_to_category_management):
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        #计算类别
        ...
        try:
            #计算现在有多少类别
            nodes=WebDriverWait(driver, timeout=5, poll_frequency=0.5).until(EC.presence_of_all_elements_located((By.XPATH,"//tbody/tr")))
            now=len(nodes)
        except TimeoutException:
            # 如果页面上没有对应的元素，直接设置now为0
            now = 0
...
@allure.feature("网站配置管理")
@allure.story("类别管理")
@allure.description("删除类别")
def test_delete(driver,navigate_to_category_management):
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        #计算类别
        ...
        if (ago-now)!=1:
            highlight_element(driver, nodes)
            allure.attach(driver.get_screenshot_as_png(), name="删除类别失败截图",
                          attachment_type=allure.attachment_type.PNG)
            reset_element(driver, nodes)
        assert (ago-now)==1
    except Exception as e:
