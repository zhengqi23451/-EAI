...
def navigate(driver, login):
    ...
    try:
        ...
        # 点击部门结构管理
        wait.until(EC.element_to_be_clickable(
            (By.XPATH, '//div[@class="sidebar-container"]//li/ul//span[text()="面试资料管理"]'))).click()
        # 验证页面加载
        time.sleep(2)
...
@allure.feature("人力资源工作")
@allure.story("面试资料管理")
@allure.description("面试资料查看")
@pytest.mark.parametrize("driver",['chrome'],indirect=True)
def test_view_interview_materials(driver, navigate):
    """测试按文件名查询"""
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        ...
        text=wait.until(EC.presence_of_element_located(
            (By.XPATH, '//form//span[text()="应聘基础资料"]')))
        n = text.text
        if "应聘基础资料" not in n :
            highlight_element(driver,text)
            allure.attach(driver.get_screenshot_as_png(), name="查询失败截图",attachment_type=allure.attachment_type.PNG)
