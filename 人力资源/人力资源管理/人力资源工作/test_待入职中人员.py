...
def navigate(driver, login):
    ...
    try:
        ...
        # 点击部门结构管理
        wait.until(EC.element_to_be_clickable(
            (By.XPATH, '//div[@class="sidebar-container"]//li/ul//span[text()="待入职中人员"]'))).click()
        # 验证页面加载
        time.sleep(2)
...
def test_search_by_name(driver, navigate):
    ...
    try:
        #部门查询测试
        wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="app"]//div[label[@for="name"]]//input'))).send_keys("红奕泽")
...
def test_search_by_job(driver, navigate):
    ...
    try:
        ...
        time.sleep(1)
        wait.until(EC.element_to_be_clickable((By.XPATH,'//div[@x-placement="bottom-start"]//li/span[text()="职员"]'))).click()
...
@allure.feature("人力资源工作")
@allure.story("待入职中人员")
@allure.description("签订状况查询")
@pytest.mark.parametrize("driver",['chrome'],indirect=True)
def test_search_by_sign(driver, navigate):
    """测试按文件名查询"""
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        #部门查询测试
        wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="app"]//div[label[@for="signing_status"]]//input'))).click()
        wait.until(EC.element_to_be_clickable((By.XPATH,'//div[@x-placement="bottom-start"]//li/span[text()="已提交"]'))).click()
...
@allure.feature("人力资源工作")
@allure.story("待入职中人员")
@allure.description("保险状况查询")
@pytest.mark.parametrize("driver",['chrome'],indirect=True)
def test_search_by_Insurance(driver, navigate):
    """测试按文件名查询"""
    wait = WebDriverWait(driver, 20)
    time.sleep(3)
    try:
        #部门查询测试
        wait.until(EC.presence_of_element_located((By.XPATH, '//*[@id="app"]//div[label[@for="insurance_status"]]//input'))).click()
        time.sleep(1)
        wait.until(EC.element_to_be_clickable((By.XPATH,'//div[@x-placement="bottom-start"]//li/span[text()="已办理"]'))).click()
